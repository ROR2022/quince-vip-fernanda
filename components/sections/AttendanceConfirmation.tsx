...
import { Phone, Heart, Sparkles, User, MessageCircle, Users, CheckCircle } from "lucide-react";
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.action === 'updated') {
                    const matchMethod = result.matchInfo?.matchMethod === 'phone' ? 'teléfono' : 'nombre';
                    const conflictInfo = result.matchInfo?.hasConflict ? ' (⚠️ números diferentes)' : '';
                    console.log(`✅ Invitado actualizado por ${matchMethod}: "${result.guest.name}" (${result.matchInfo?.similarity?.toFixed(1)}% similitud)${conflictInfo}`);
                    
                    if (result.matchInfo?.hasConflict) {
                        console.log(`⚠️ Se detectó un conflicto de teléfono - verificar manualmente`);
                    }
                } else if (result.action === 'created') {
                    console.log(`🆕 Nuevo invitado creado: "${result.guest.name}"`);
                    if (result.matchInfo?.multipleMatches) {
                        console.log(`⚠️ Búsqueda ambigua: ${result.matchInfo.matchesCount} coincidencias similares encontradas`);
                    }
                }
            } else {
    ...
    className="relative py-20 px-4">
      ...
      <div className="max-w-2xl mx-auto relative">
        <div
          ...
          style={{
            background: "linear-gradient(135deg, rgba(255, 179, 217, 0.08) 0%, rgba(248, 246, 240, 0.95) 25%, rgba(230, 217, 255, 0.08) 50%, rgba(255, 242, 204, 0.12) 75%, rgba(253, 252, 252, 0.95) 100%)",
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xl leading-relaxed max-w-lg mx-auto" 
               style={{ color: "var(--color-aurora-rosa)" }}>
              ¿Nos acompañarás en este día tan especial?<br />
              <span className="font-medium">Confirma tu asistencia y comparte este momento único</span>
            </p>
          </div>
          ...
          <form onSubmit={handleConfirmAttendance} className="space-y-6">
            {/* Mensaje de éxito */}
            {showSuccess && (
              <div className="text-center p-4 rounded-2xl mb-6 animate-pulse"
                   style={{ 
                     background: "linear-gradient(135deg, rgba(255, 179, 217, 0.2), rgba(230, 217, 255, 0.2))",
                ...
                <div className="relative flex items-center justify-center">
                  {isSubmitting ? (
                    ...
                    <>
                      ...
                      <h6 className="text-purple-700 flex flex-col">
                        <span>✨ Confirmar</span>  
